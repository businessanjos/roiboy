  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="bg-zapp-input border-zapp-border text-zapp-text max-w-lg">
          <DialogHeader>
          ...
          <ScrollArea className="h-72">
            {quickReplies.length === 0 ? (
              <div className="text-center py-8">
                <Zap className="h-12 w-12 mx-auto text-zapp-text-muted mb-3" />
                <p className="text-zapp-text-muted">Nenhuma resposta rápida criada</p>
                <p className="text-zapp-text-muted text-sm">Crie respostas para agilizar seu atendimento</p>
              </div>
            ) : (
              <div className="space-y-2">
                {quickReplies.map((reply) => (
                  <div
                    key={reply.id}
                    className="group flex items-start gap-2 p-3 rounded-lg hover:bg-zapp-panel transition-colors"
                  >
                    <button
                      onClick={() => onUseReply(reply)}
                      className="flex-1 text-left"
                    >
                      <p className="text-zapp-text font-medium">{reply.title}</p>
                      <p className="text-zapp-text-muted text-sm line-clamp-2">{reply.content}</p>
                    </button>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button
                        size="icon"
                        variant="ghost"
                        className="h-7 w-7 text-zapp-text-muted hover:text-zapp-text"
                      ...
                      <Button
                        size="icon"
                        variant="ghost"
                        className="h-7 w-7 text-zapp-text-muted hover:text-danger"
      ...
      <Dialog open={editDialogOpen} onOpenChange={onEditDialogChange}>
        <DialogContent className="bg-zapp-input border-zapp-border text-zapp-text">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="qr-title" className="text-zapp-text-muted">Título</Label>
              <Input
                id="qr-title"
                value={form.title}
            ...
            <div className="space-y-2">
              <Label htmlFor="qr-content" className="text-zapp-text-muted">Conteúdo da mensagem</Label>
              <Textarea
                id="qr-content"
                value={form.content}
                ...
                rows={4}
                className="bg-zapp-panel border-zapp-border text-zapp-text font-mono text-sm"
              />
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => onEditDialogChange(false)} className="border-zapp-border text-zapp-text-muted">
              Cancelar
            </Button>
