...
import { Plus, Trash2, GripVertical, Pencil, X, CheckCircle2, ListChecks, Calendar, Hash, Type, ToggleLeft, Users, Instagram } from "lucide-react";
...
interface LeadCustomFieldsManagerProps {
  onFieldsChange?: () => void;
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
}

export function LeadCustomFieldsManager({ onFieldsChange, open: externalOpen, onOpenChange: externalOnOpenChange }: LeadCustomFieldsManagerProps) {
  ...
  const [fieldType, setFieldType] = useState<LeadCustomField["field_type"]>("select");
  ...
  const fetchFields = async () => {
    ...
    if (!error && data) {
      const mappedFields: LeadCustomField[] = data.map(f => ({
        id: f.id,
        name: f.name,
  ...
  const openEditDialog = (field: LeadCustomField) => {
  ...
  const handleDelete = async (fieldId: string) => {
    if (!confirm("Tem certeza que deseja excluir este campo? Os valores dos leads serão perdidos.")) {
      return;
    }
  ...
  const content = (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        {!isControlled && (
          <div>
            <h3 className="text-lg font-medium">Campos Personalizados de Leads</h3>
            <p className="text-sm text-muted-foreground">
              Crie campos para acompanhar informações dos leads
            </p>
          </div>
        )}
            ...
            <div className="space-y-5 py-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="Temperatura, interesse..."
                    value={name}
                ...
                <div className="space-y-2">
                  ...
                  <Select value={fieldType} onValueChange={(v) => setFieldType(v as LeadCustomField["field_type"])}>
                    <SelectTrigger>
              ...
              <div className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Exigir preenchimento ao criar lead</p>
                </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-muted-foreground">
          Nenhum campo personalizado criado para leads
        </div>
      ) : (
        <DndContext
          sensors={sensors}
