...
const COMMISSION_MODELS: CommissionModel[] = [
  { id: 'scaled', name: 'Escalonado', description: 'Percentual varia de acordo com a faixa de atingimento da meta.', icon: TrendingUp, color: 'text-success', bgColor: 'bg-success/10', bestFor: ['Equipes com incentivo progressivo', 'Empresas com metas claras'], pros: ['Incentiva superação de metas', 'Justo para diferentes perfis'], cons: ['Complexo de calcular', 'Requer definição clara de faixas'] },
  { id: 'recurring', name: 'Recorrente', description: 'Comissão paga mensalmente enquanto o cliente mantiver contrato.', icon: ArrowUpRight, color: 'text-info', bgColor: 'bg-info/10', bestFor: ['SaaS e assinatura', 'Serviços recorrentes'], pros: ['Incentiva retenção', 'Renda previsível'], cons: ['Atrasa ganhos imediatos'] },
  ...
  { id: 'ote', name: 'OTE', description: 'Salário fixo + variável. Transparência total.', icon: Target, color: 'text-warning', bgColor: 'bg-warning/10', bestFor: ['Atração de talentos', 'Vendas consultivas B2B'], pros: ['Transparência total', 'Atrai talentos'], cons: ['Custo fixo elevado'] },
  { id: 'pure', name: 'Comissão Pura', description: 'Baixo fixo, alta variável.', icon: Zap, color: 'text-danger', bgColor: 'bg-danger/10', bestFor: ['Varejo', 'Alto volume'], pros: ['Baixo custo fixo', 'Sem limite de ganhos'], cons: ['Alta rotatividade'] },
  { id: 'gross_revenue', name: 'Por Faturamento Bruto', description: 'Comissão sobre faturamento bruto total.', icon: BarChart3, color: 'text-info', bgColor: 'bg-info/10', bestFor: ['Margens altas', 'Produtos padronizados'], pros: ['Simples de calcular'], cons: ['Ignora rentabilidade'] },
  { id: 'profit_margin', name: 'Por Margem de Lucro', description: 'Comissão baseada na margem, não no faturamento.', icon: Shield, color: 'text-success', bgColor: 'bg-success/10', bestFor: ['Margens variáveis', 'Vendas consultivas'], pros: ['Protege margens'], cons: ['Mais complexo'] },
  { id: 'bonus', name: 'Modelo de Bônus', description: 'Remuneração variável por metas específicas.', icon: Award, color: 'text-warning', bgColor: 'bg-warning/10', bestFor: ['Crescimento acelerado', 'Performance excepcional'], pros: ['Incentiva alta performance', 'Flexível'], cons: ['Pode frustrar se metas forem irrealistas'] },
];
                    ...
                    {isSelected && (
                      <CardContent className="pt-0 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div><p className="text-xs font-semibold text-success dark:text-success mb-1">Vantagens</p><ul className="space-y-0.5">{model.pros.map((p, i) => <li key={i} className="text-xs text-muted-foreground flex items-start gap-1"><ChevronRight className="w-3 h-3 text-success mt-0.5 shrink-0" />{p}</li>)}</ul></div>
                          <div><p className="text-xs font-semibold text-danger dark:text-danger mb-1">Desvantagens</p><ul className="space-y-0.5">{model.cons.map((c, i) => <li key={i} className="text-xs text-muted-foreground flex items-start gap-1"><ChevronRight className="w-3 h-3 text-danger mt-0.5 shrink-0" />{c}</li>)}</ul></div>
                        </div>
            ...
            <div className="space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Separator />
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5"><Label className="text-xs flex items-center gap-1"><ArrowUpRight className="w-3 h-3 text-success" />Acelerador</Label><Input type="number" step="0.1" value={dn(quota.acceleratorRate)} onChange={e => setQuota(p => ({ ...p, acceleratorRate: pn(e.target.value) }))} /></div>
                    <div className="space-y-1.5"><Label className="text-xs flex items-center gap-1"><ArrowDownRight className="w-3 h-3 text-danger" />Desacelerador</Label><Input type="number" step="0.1" value={dn(quota.deceleratorRate)} onChange={e => setQuota(p => ({ ...p, deceleratorRate: pn(e.target.value) }))} /></div>
                  </div>
