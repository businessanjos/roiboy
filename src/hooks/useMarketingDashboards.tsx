...
export function useMarketingDashboards() {
  const { currentUser } = useCurrentUser();
  const queryClient = useQueryClient();
  const [activeDashboardId, setActiveDashboardId] = useState<string | null>(null);

  // Fetch dashboards filtered by sector = 'marketing'
  const { data: dashboards = [], isLoading } = useQuery({
    queryKey: ["marketing-dashboards", currentUser?.account_id],
    queryFn: async () => {
  ...
  const { data: visuals = [], isLoading: isLoadingVisuals } = useQuery({
    queryKey: ["marketing-visuals", effectiveId],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (name: string) => {
      ...
      const { data, error } = await supabase
        .from("insights_dashboards")
        .insert({ name, account_id: currentUser.account_id, user_id: currentUser.id, folder: "Marketing", sector: "marketing" } as any)
        .select()
        .single();
      if (error) throw error;
      return data as MarketingDashboard;
    ...
    onSuccess: (d) => {
      queryClient.invalidateQueries({ queryKey: ["marketing-dashboards"] });
  ...
  const addVisualMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["marketing-visuals", effectiveId] });
  ...
  const updateVisualMutation = useMutation({
    ...
    onSuccess: ({ id, updates }) => {
      ...
      if (isLayoutOnly) {
        queryClient.setQueryData(["marketing-visuals", effectiveId], (old: InsightsVisual[] | undefined) => {
          if (!old) return old;
