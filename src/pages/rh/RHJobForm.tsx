...
function AutosaveIndicator({ status, lastSavedAt }: { status: "idle" | "saving" | "saved" | "error"; lastSavedAt: Date | null }) {
  ...
  if (status === "error") return <span className="inline-flex items-center gap-1.5 text-danger"><AlertCircle className="h-3.5 w-3.5" />Erro ao salvar</span>;
  if (status === "saved" && lastSavedAt) return <span className="inline-flex items-center gap-1.5 text-success"><Check className="h-3.5 w-3.5" />Salvo {formatDistanceToNow(lastSavedAt, { locale: ptBR, addSuffix: true })}</span>;
  return null;
}
