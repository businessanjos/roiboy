...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch("https://app.omie.com.br/api/v1/geral/contacorrente/", {
      method: "POST",
    ...
    const accounts = (result.ListarContasCorrentes || []).map((cc: any) => ({
      nCodCC: cc.nCodCC,
      descricao: cc.descricao || cc.cDescricao || `Conta ${cc.nCodCC}`,
    }));
