DROP FUNCTION IF EXISTS public.get_ops_consultant_workload(integer, timestamptz, timestamptz);
...
RETURNS TABLE (
  ...
  clients_attended integer,
  inbound_msgs integer,
  outbound_msgs integer,
  conversations integer,
  avg_first_response_min numeric,
  median_first_response_min numeric,
  total_response_time_min numeric,
...
AS $$
  ...
  msgs AS (
    ...
    WHERE cl.responsible_user_id IN (SELECT id FROM ops_users)
      AND cl.id IN (SELECT client_id FROM clients_with_active_contract)
