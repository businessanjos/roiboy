CREATE OR REPLACE FUNCTION public.zapp_can_view_analytics(_user_id uuid DEFAULT auth.uid())
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    SELECT 1
    FROM public.users u
    LEFT JOIN public.user_team_roles utr ON utr.user_id = u.id
    LEFT JOIN public.team_roles tr ON tr.id = utr.team_role_id
    WHERE u.auth_user_id = _user_id
      AND (
        u.role IN ('admin', 'super_admin')
        OR u.is_also_admin = true
        OR tr.name ILIKE '%head%'
        OR tr.name ILIKE '%diretor%'
        OR tr.name ILIKE '%sócio%'
        OR tr.name ILIKE '%socio%'
        OR (u.role = 'gestor' AND tr.name ILIKE 'admin%')
      )
  );
$$;
  ...
  SELECT jsonb_build_object(
    ...
    'by_agent', (
      SELECT coalesce(jsonb_agg(x ORDER BY (x->>'messages_sent')::int DESC), '[]'::jsonb) FROM (
        SELECT jsonb_build_object(
          'user_id', u.id,
          'name', coalesce(u.name, 'Sem atendente'),
          'messages_sent', count(*),
          'conversations', count(DISTINCT m.cid),
          'avg_response_seconds', (SELECT round(avg(r.secs)) FROM responses r WHERE r.sender_user_id = m.sender_user_id)
        ) AS x
        FROM msgs m
    ...
    'by_day', (
      SELECT coalesce(jsonb_agg(x ORDER BY x->>'day'), '[]'::jsonb) FROM (
        SELECT jsonb_build_object(
          'day', to_char(date_trunc('day', m.sent_at AT TIME ZONE 'America/Sao_Paulo'), 'YYYY-MM-DD'),
    ...
    'by_hour', (
      SELECT coalesce(jsonb_agg(x ORDER BY (x->>'hour')::int), '[]'::jsonb) FROM (
        SELECT jsonb_build_object(
          'hour', EXTRACT(HOUR FROM m.sent_at AT TIME ZONE 'America/Sao_Paulo')::int,
          'inbound', count(*) FILTER (WHERE m.direction = 'inbound'),
          'outbound', count(*) FILTER (WHERE m.direction = 'outbound')
        ) AS x
        FROM msgs m
...
GRANT EXECUTE ON FUNCTION public.zapp_can_view_analytics(uuid) TO authenticated;
