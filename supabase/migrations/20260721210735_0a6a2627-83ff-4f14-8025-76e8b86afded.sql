-- FIX: renegotiate_installment estava tentando inserir installment_events com
-- event_type='renegotiated', mas a constraint só aceita 'renegotiation'.
-- Isso quebrava 100% das renegociações via RPC (rollback da transação inteira).
CREATE OR REPLACE FUNCTION public.renegotiate_installment(p_installment_id uuid, p_reason text, p_new_installments jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_new_installments) LOOP
    ...
    INSERT INTO public.installments (
      ...
      v_account_id, v_original.invoice_id, v_row_number, v_row_due, v_row_amount, v_row_method,
      'pending', v_original.id, auth.uid(),
  ...
  -- Evento na parcela original — FIX: event_type deve ser 'renegotiation' (constraint installment_events_event_type_check)
  INSERT INTO public.installment_events (
    account_id, installment_id, invoice_id, event_type, payload, visible_to, created_by
  ) VALUES (
    v_original.account_id, v_original.id, v_original.invoice_id, 'renegotiation',
    jsonb_build_object(
      'reason', p_reason,
