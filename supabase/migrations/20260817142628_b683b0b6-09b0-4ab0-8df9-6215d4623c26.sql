...
BEGIN
  ...
  -- Mensagens do período (base da maioria das métricas)
  msgs AS (
  ...
  -- Janela estendida (48h antes) para não perder o pareamento de resposta na virada do período
  msgs_ext AS (
    SELECT m.id, m.zapp_conversation_id AS cid, m.direction, m.sent_at, m.sender_user_id
    FROM public.zapp_messages m
    JOIN convs c ON c.id = m.zapp_conversation_id
    WHERE m.account_id = _account
      AND m.sent_at >= (_from - interval '48 hours') AND m.sent_at < _to
      AND coalesce(m.is_deleted, false) = false
  ),
  first_ever AS (
    SELECT DISTINCT ON (m.zapp_conversation_id)
      m.zapp_conversation_id AS cid, m.direction, m.sent_at, m.sender_user_id,
      coalesce(m.synced_from_history, false) AS from_history
    FROM public.zapp_messages m
    JOIN convs c ON c.id = m.zapp_conversation_id
    WHERE m.account_id = _account
      AND coalesce(m.is_deleted, false) = false
    ORDER BY m.zapp_conversation_id, m.sent_at ASC, m.id ASC
  ),
  -- Conversa nova de verdade: 1ª mensagem no período, não importada de histórico
  -- e a conversa também foi criada no sistema dentro do período.
  new_candidates AS (
  ...
  -- Última mensagem REAL da conversa (sem recorte de período) para "sem resposta"
  last_msg AS (
    SELECT DISTINCT ON (m.zapp_conversation_id)
      m.zapp_conversation_id AS cid, m.direction, m.sent_at
    FROM public.zapp_messages m
    JOIN (SELECT DISTINCT cid FROM msgs) a ON a.cid = m.zapp_conversation_id
    WHERE m.account_id = _account
      AND coalesce(m.is_deleted, false) = false
    ORDER BY m.zapp_conversation_id, m.sent_at DESC, m.id DESC
  ),
  totals AS (
    SELECT
      ...
      WHERE cl.account_id = _account
        ...
        AND NOT EXISTS (
          ...
          WHERE zc.client_id = cl.id AND zc.account_id = _account
            ...
            AND (_integration_id IS NULL OR zc.integration_id = _integration_id)
        )
    ),
    'risk_mentions', (
...
GRANT EXECUTE ON FUNCTION public.zapp_productivity_metrics(text, timestamptz, timestamptz, uuid, boolean) TO authenticated;
