...
CREATE OR REPLACE FUNCTION public.contract_is_mvp(_contract_id uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.client_contracts cc
    JOIN public.products p ON p.id = cc.product_id
    WHERE cc.id = _contract_id AND p.name ILIKE '%MVP%'
  );
$$;
...
BEGIN
  ...
  FOR v_i IN 1..v_total LOOP
    ...
    IF v_is_mvp AND v_row_method = 'credit_card' THEN
      v_status := 'paid';
...
-- Backfill: parcelas MVP em cartão => pagas
...
SET status = 'paid',
    paid_at = COALESCE(i.paid_at, i.due_date, CURRENT_DATE),
    paid_amount = i.amount
FROM public.invoices inv
WHERE inv.id = i.invoice_id
  AND inv.contract_id IN (SELECT id FROM mvp_contracts)
  AND i.payment_method = 'credit_card'
  AND i.status IN ('pending','overdue')
  AND COALESCE(i.paid_amount,0) = 0;

-- Backfill FE correspondentes: para contratos MVP, quitar FE cujos installments
-- correspondentes (por invoice_id equivalente ao contract) estão em cartão.
WITH mvp_contracts AS (
  SELECT cc.id FROM public.client_contracts cc
  JOIN public.products p ON p.id = cc.product_id
  WHERE p.name ILIKE '%MVP%'
...
card_inst AS (
  SELECT DISTINCT inv.contract_id, i.due_date, i.amount
  FROM public.installments i
  JOIN public.invoices inv ON inv.id = i.invoice_id
  WHERE inv.contract_id IN (SELECT id FROM mvp_contracts)
    AND i.payment_method = 'credit_card'
)
UPDATE public.financial_entries fe
SET status = 'paid',
    payment_date = COALESCE(fe.payment_date, fe.due_date, CURRENT_DATE)
FROM card_inst ci
WHERE fe.contract_id = ci.contract_id
  AND fe.entry_type = 'receivable'
  AND fe.due_date = ci.due_date
  AND ABS(fe.amount - ci.amount) < 0.01
