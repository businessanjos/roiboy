CREATE OR REPLACE FUNCTION public.zapp_productivity_metrics(_sector_id text, _from timestamp with time zone, _to timestamp with time zone, _integration_id uuid DEFAULT NULL::uuid, _include_groups boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  client_scope AS (
    SELECT cb.id,
      EXISTS (SELECT 1 FROM convs c WHERE c.client_id = cb.id) AS has_conv,
      EXISTS (
        SELECT 1 FROM convs c
        JOIN dedup d ON d.cid = c.id AND d.direction = 'inbound'
