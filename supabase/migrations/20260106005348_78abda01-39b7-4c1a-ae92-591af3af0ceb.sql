-- Drop the existing check constraint and add a new one with instagram type
...
CHECK (field_type IN ('select', 'boolean', 'multi_select', 'number', 'currency', 'text', 'date', 'user', 'instagram'));