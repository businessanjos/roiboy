...
-- Update the trigger function to include royzapp.access permission
...
BEGIN
  ...
  -- Add default permissions for Admin role (all permissions including royzapp.access)
  INSERT INTO public.role_permissions (role_id, permission)
  SELECT r.id, p.permission
  FROM public.team_roles r,
  UNNEST(ARRAY['clients.view', 'clients.edit', 'clients.delete', 'team.view', 'team.edit', 'settings.view', 'settings.edit', 'reports.view', 'events.view', 'events.edit', 'forms.view', 'forms.edit', 'products.view', 'products.edit', 'royzapp.access']) AS p(permission)
  WHERE r.account_id = NEW.id AND r.name = 'Admin';

  -- Add default permissions for other roles (including royzapp.access)
  INSERT INTO public.role_permissions (role_id, permission)
  SELECT r.id, p.permission
  FROM public.team_roles r,
  UNNEST(ARRAY['clients.view', 'clients.edit', 'team.view', 'reports.view', 'events.view', 'forms.view', 'products.view', 'royzapp.access']) AS p(permission)
  WHERE r.account_id = NEW.id AND r.name IN ('Head', 'Gestor', 'Mentor');

  INSERT INTO public.role_permissions (role_id, permission)
  SELECT r.id, p.permission
  FROM public.team_roles r,
  UNNEST(ARRAY['clients.view', 'clients.edit', 'reports.view', 'events.view', 'forms.view', 'royzapp.access']) AS p(permission)
