...
DECLARE
  ...
  v_responsible    uuid;
BEGIN
  IF NEW.direction::text NOT IN ('inbound', 'client_to_team') THEN
    RETURN NEW;
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  -- Nunca bloquear a gravação da mensagem por falha de roteamento
  RAISE WARNING 'auto_route_client_conversation_to_cs falhou: %', SQLERRM;
  RETURN NEW;
END;
$function$;