-- Fix: Make the dispatch_ryka_contract_event trigger not fail when http_post is unavailable
CREATE OR REPLACE FUNCTION dispatch_ryka_contract_event()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE WARNING 'dispatch_ryka_contract_event: http_post failed: %', SQLERRM;
  END;
