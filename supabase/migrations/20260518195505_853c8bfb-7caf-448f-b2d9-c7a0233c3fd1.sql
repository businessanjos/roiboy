CREATE OR REPLACE FUNCTION public.get_ops_consultant_workload(p_days integer DEFAULT 7)
RETURNS TABLE (
  user_id uuid,
  name text,
  email text,
...
AS $$
  ...
  since AS (SELECT (now() - make_interval(days => p_days)) AS ts),
  active AS (
  ...
  msgs AS (
    ...
    WHERE cl.responsible_user_id IN (SELECT id FROM ops_users)
      AND m.sent_at >= (SELECT ts FROM since)
  ),
  inbound_agg AS (
    SELECT uid,
  ...
  SELECT
    ...
    coalesce(a.n, 0),
    coalesce(ia.clients_msg, 0),
    coalesce(ia.inbound, 0),
    coalesce(oa.outbound, 0),
    coalesce(ac.convs, 0),
    coalesce(ra.avg_min, 0),
    coalesce(ra.responded, 0),
    coalesce(ra.total_in, 0)
  FROM ops_users u
...
REVOKE ALL ON FUNCTION public.get_ops_consultant_workload(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_ops_consultant_workload(integer) TO authenticated;