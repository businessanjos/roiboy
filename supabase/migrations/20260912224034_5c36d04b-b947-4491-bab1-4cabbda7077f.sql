...
BEGIN
  WITH opts AS (
    SELECT cf.id AS field_id, jsonb_array_elements(cf.options) o
    FROM public.custom_fields cf
    WHERE cf.name = 'Origem da Venda' AND cf.show_in_deals = true
  ), tv AS (
    SELECT DISTINCT field_id, o->>'value' AS v FROM opts
    WHERE upper(o->>'label') LIKE '%TRAF-%'
       OR upper(o->>'label') LIKE '%ORG-%'
       OR upper(o->>'label') LIKE '%INSTAGRAM%'
       OR upper(o->>'label') LIKE '%PODCAST%'
  ), td AS (
    SELECT DISTINCT dfv.deal_id
    FROM public.deal_field_values dfv
    WHERE EXISTS (
      SELECT 1 FROM tv WHERE tv.field_id = dfv.field_id AND tv.v = ANY(
