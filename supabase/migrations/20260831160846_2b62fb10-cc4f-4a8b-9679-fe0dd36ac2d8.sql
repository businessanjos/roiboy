...
CREATE OR REPLACE FUNCTION public.zapp_phone_core(_phone text)
RETURNS text
LANGUAGE sql
IMMUTABLE
SET search_path TO 'public'
AS $$
  WITH d AS (SELECT regexp_replace(coalesce(_phone, ''), '\D', '', 'g') AS n),
  s AS (
    SELECT CASE
      WHEN length(n) >= 12 AND left(n, 2) = '55' THEN substr(n, 3)
      ELSE n
    END AS n
    FROM d
  )
  SELECT CASE
    WHEN length(n) < 10 THEN nullif(n, '')
...
AS $$
  SELECT CASE
    WHEN public.zapp_phone_core(_phone) IS NULL THEN false
    ELSE EXISTS (
      SELECT 1 FROM public.zapp_internal_contacts i
      WHERE i.account_id = _account
        AND public.zapp_phone_core(i.phone_e164) = public.zapp_phone_core(_phone)
    ) OR EXISTS (
      SELECT 1 FROM public.hr_collaborators h
      WHERE h.account_id = _account
        AND h.phone IS NOT NULL
        AND public.zapp_phone_core(h.phone) = public.zapp_phone_core(_phone)
    )
  END;
$$;

-- Um contato só é considerado interno se NÃO for cliente com contrato ativo
CREATE OR REPLACE FUNCTION public.zapp_should_exclude_contact(_account uuid, _phone text, _client_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
  SELECT public.zapp_is_internal_contact(_account, _phone)
     AND NOT EXISTS (
       SELECT 1 FROM public.client_contracts cc
       WHERE cc.client_id = _client_id AND cc.status = 'active'
     );
$$;
...
CREATE OR REPLACE FUNCTION public.zapp_productivity_contacts(_sector_id text, _from timestamp with time zone, _to timestamp with time zone, _integration_id uuid DEFAULT NULL::uuid, _include_groups boolean DEFAULT false, _agent_user_id uuid DEFAULT NULL::uuid, _direction text DEFAULT 'outbound'::text)
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  WITH convs_raw AS (
    SELECT c.id, c.client_id, c.is_group, c.contact_name, c.phone_e164, c.created_at,
           public.zapp_phone_core(c.phone_e164) AS pcore
    ...
    WHERE c.account_id = _account
      ...
      AND NOT public.zapp_should_exclude_contact(_account, c.phone_e164, c.client_id)
  ),
  convs AS (
  ...
  scope AS (
    SELECT
      coalesce(c.client_id::text, c.pcore, c.id::text) AS contact_key,
...
BEGIN
  ...
  client_base AS (
    SELECT DISTINCT cl.id, public.zapp_phone_core(cl.phone_e164) AS pcore
  ...
  client_scope AS (
    SELECT cb.id,
      EXISTS (
        ...
        WHERE c.client_id = cb.id OR (cb.pcore IS NOT NULL AND c.pcore = cb.pcore)
      ) AS has_conv,
      EXISTS (
        SELECT 1 FROM convs_raw c
  ...
  SELECT jsonb_build_object(
    ...
    'contacts_reached_by_team', (
      SELECT count(DISTINCT coalesce(c.client_id::text, c.pcore, m.cid::text))
    ...
    'silent_conversations', (
      SELECT count(*) FROM (SELECT DISTINCT cid FROM msgs) a
      WHERE NOT EXISTS (SELECT 1 FROM msgs m WHERE m.cid = a.cid AND m.direction = 'inbound')
    ),
